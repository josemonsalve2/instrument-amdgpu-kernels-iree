...
std::string LoadOrStoreMap(const BasicBlock::iterator &I){
		...
		else printf("Error: unknown operation type");
    return "";
}

  static uint64_t NoDebCounter = 0;
...
void InjectingInstrumentationFunction(const BasicBlock::iterator &I, const Function &F, const llvm::Module &M,
  ...
  if (!DL) {
    std::string NoDebCounterStr = std::to_string(NoDebCounter);
    SourceAndAddrSpaceInfo = (F.getName() + "     " + "unknown:" + NoDebCounterStr + ":0" + "     " + AddrSpaceMap[AddrSpace] + "     " + LoadOrStoreMap(I)).str();
    NoDebCounter++;
  } else {
        SourceAndAddrSpaceInfo =
	...
	if(LocationCounterSourceMap.find(SourceAndAddrSpaceInfo) == LocationCounterSourceMap.end()){
        ...
        printf("Instrumentation Function found: %s\n", InstrumentationFunction->getName().str().c_str());
...
bool AMDGCNMemTrace::runOnModule(Module &M) {
  ...
  // Check that InstrumentationFunctionFile is not empty
  if (InstrumentationFunctionFile.empty()) {
    printf("error: environment variable AMDCGN_INSTRUMENTATION_FUNCTIONS_FILE is not set\n");
    exit(1);
  }
  ...
  for (auto &F : M) {
    ...
    if (F.getCallingConv() == CallingConv::AMDGPU_KERNEL) {
      ...
      // We will need to remove attributes. 
    } 
}
